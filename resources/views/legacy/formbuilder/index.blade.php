<script>
	...
	$(function() {
			...
			$("#drop_area ol").droppable({
				...
				drop: function(event, ui) {
					...
					if (question_type_id == 25) {
						...
						html = html + '<a href="javascript:;" onclick="remove_div(' + random_id + ')"><img src="{{ rtrim(url("/"), "/") }}/form-builder/images/close-button.png" border="0" alt="Remove" title="Remove" /></a>';
					} else {
					...
					switch (question_type_id) {
						...
						case "11": // image
							...
							html = html + '<iframe src="<?php echo rtrim(url('/'), '/').'/'; ?>test/temp_image_upload?profile_id=<?php echo $profile_id; ?>" id="logoUpload" scrolling="no" frameborder="0" width="94%" style="padding-left:10px" height="50"></iframe>';
						...
						case "21": // Document Button
							...
							html = html + '<iframe src="<?php echo rtrim(url('/'), '/').'/'; ?>test/temp_doc_upload?profile_id=<?php echo $profile_id; ?>&random_id=' + random_id + '" id="docUpload" scrolling="no" frameborder="0" width="94%" style="padding-left:10px" height="50"></iframe>';
						...
						case "22": // SWMS Hazard/Risk
							...
							html = html + '<div style="display: flex;align-items: center;min-width: 70px;margin-left:15px;"><img src="{{ rtrim(url("/"), "/") }}/form-builder/images/camera.png" style="height:25px;width:25px;" />&nbsp;<span stye="margin-top:5px !important;">Photo</span></div>';
						...
						case "23": // Document Menu
							...
							html = html + '<iframe src="<?php echo rtrim(url('/'), '/').'/'; ?>test/temp_doc_multi_upload?profile_id=<?php echo $profile_id; ?>&random_id=' + random_id + '" id="docUpload" scrolling="no" frameborder="0" width="94%" style="padding-left:10px" height="50"></iframe>';
							break;
					...
					setTimeout(function() {
						if (window.parent.$('span.expand-reduce').text() == "Reduce Window") {
							ul_scroll_height = $('.ui-droppable').prop('scrollHeight') + 10;
							$('#div_drop_area').css('height', ul_scroll_height);
							window.parent.$('#iframe_frm_builder').height(ul_scroll_height + $('.top-part').height());
						}
				...
				update: function(event, ui) {
					...
					$.ajax({
						type: "GET",
						url: "{{ url("/portal/legacy-form-builder/reorder-element") }}",
						dataType: "html",
						data: {
		...
		window.parent.$('#iframe_frm_builder').height(iframe_height + (75 * email_recipient_cnt));
		...
		window.parent.$('#enable_form').on('click', function() {
			...
			$.ajax({
				type: "GET",
				url: "{{ url("/portal/legacy-form-builder/update-enable-form") }}",
				dataType: "html",
				data: {
					"profile_id": '<?php echo $profile_id; ?>',
		...
		$('#add_edit_participant_list').on('click', function() {
			// Legacy: opens parent Colorbox (#participant_list_tag). Portal: ask parent to open Participants.
		...
		$('.help-div a').on('click', function(e) {
			...
			if (url != '') {
				...
				if (result) {
					parent.$('#how_to_pop_up').html('<iframe src="' + url + '" frameborder="0" allowfullscreen wmode="transparent" width="480" height="300" ></iframe>');
					parent.$('.black-tint').show();
					parent.$('.player-window').show(500);
				} else {
					var substr = 'image#';
					if (url.indexOf(substr) !== -1) {
						url = url.replace(substr, "");
						parent.$('.black-tint').show();
						parent.$('.player-window-image').show(500);
					} else {
						return false;
					}
				}
		...
		<div class="from-box">
			...
			<div id="drop_area">
				<div class="progressbar">
					<img align="absmiddle" src="{{ rtrim(url("/"), "/") }}/images/spinner_gray.gif"> Please Wait...
				</div>
...
<script>
	function add_another_recipient(id) {
		...
		window.parent.$('#iframe_frm_builder').height(iframe_height + 75);
	...
	function remove_recipient(obj) {
		...
		window.parent.$('#iframe_frm_builder').height(iframe_height - 65);
	...
	function append_div(obj, txtbox2, question_type_id, random_id, box_class, control_name, question_id) {
		...
		if (parent.$('#enable_form').is(":checked") == false) {
		...
		if (window.parent.$('#enable_form').is(':checked') == true) {
			enable_form = 1;
		} else {
			enable_form = 0;
		}
		...
		$.ajax({
			type: "GET",
			url: "{{ url("/portal/legacy-form-builder/render-element") }}",
			dataType: "html",
			data: {
				"profile_id": '<?php echo $profile_id; ?>',
	...
	function edit_element(ele_obj, ele_val, question_type_id, class_name, control_name, random_id, scale_from, scale_to, row_text, column_text, image_title, image_align, mandatory_field, button_link_url, button_colour, doc_title, include_name, include_employer, include_email, include_phone, participant_include_signature, participant_include_employer, is_logchecked, log_columntitle, text_color, bg_color) {
		...
		// 	html += '				<img src="{{ rtrim(url("/"), "/") }}/form-builder/images/close-button.png" border="0" alt="Remove" title="Remove" />';
		// 	html += '			</a>';
		// }
			...
			case 23: // Document Menu
				...
				if (doc_uploaded == "") {
					...
					for (var i = 0; i < myarr.length; i++) {
						html = html + '<a target="_blank" class="image-doc-link" href="<?php echo url('/images/formbuilder_upload/'; ?>' + myarr[i] + '">' + myarr[i] + '</a><br>';
					}
					html = html + '<br>';
				} else {
	...
	function remove_div_and_data(obj, question_id) {
		...
		$.ajax({
			type: "GET",
			url: "{{ url("/portal/legacy-form-builder/remove-element") }}",
			dataType: "html",
			data: {
				'question_id': question_id
			},
